  ...
  tbody.innerHTML = menuItems
    .map(
      (item) => `
        ...
        item.name
      }" class="w-12 h-12 rounded-lg mr-4 object-cover"; this.style.backgroundColor='#f3f4f6';">
                    <div>
                        <div class="text-sm font-medium text-gray-900">${
                          item.name
                        }</div>
                        <div class="text-sm text-gray-500">${item.desc.substring(
                          0,
                          50
                        )}...</div>
                    </div>
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                ...
                ${
                  item.originalPrice
                    ? `<span class="line-through text-gray-500 ml-2">₹${item.originalPrice}</span>`
                    : ""
                }
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
...
document.getElementById("menu-form").addEventListener("submit", async (e) => {
  ...
  const formData = {
    ...
    originalPrice:
      parseFloat(document.getElementById("item-original-price").value) ||
      undefined,
